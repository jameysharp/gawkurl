        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Ok(response) = request.send().await {
                    ...
                    let date =
                        parse::<HttpDate>(headers, header::DATE).map_or(response_time, Into::into);
                    ...
                    if response.status() == StatusCode::OK {
                        ...
                        if last_hash != hash {
                            ...
                            if let Some(last_modified) = last_modified {
                                if let Ok(last_modified_age) =
                                    date.duration_since(last_modified.into())
                                {
                                    let last_modified_age =
                                        last_modified_age.saturating_add(Duration::from_secs(age));
                                    if let Ok(last_fetched_age) =
                                        request_time.duration_since(last_fetched)
                                    {
